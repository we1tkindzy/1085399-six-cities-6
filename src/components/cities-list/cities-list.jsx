    ...
    <ul className="locations__list tabs__list">
      {CityName.map((cityName, i) => (
        <li key={cityName + i} className="locations__item">
          <a data-testid={`locations-link-${cityName}`} onClick={() => menuClickhandler(cityName)} className={`locations__item-link tabs__item ${cityName === city ? `tabs__item--active` : ``}`} href="#">
            <span>{cityName}</span>
          </a>
        </li>
      )
